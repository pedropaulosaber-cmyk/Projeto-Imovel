...
describe('vocabulário por nível', () => {
   ...
   * regresso silencioso. Subiu de 30 a 150, um lote por vez.
   */
  it('todo nível de todo idioma tem ao menos 150 verbetes', () => {
    for (const language of SUPPORTED_LANGUAGES) {
      for (const level of CEFR_LEVELS) {
        expect({
          language,
        ...
        expect(levelVocabulary(language, level).length).toBeGreaterThanOrEqual(150);
      }
    }
  });
