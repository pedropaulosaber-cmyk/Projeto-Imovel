 ...
 *
 * ## Por que este arquivo não é o `prisma/seed.ts` "oficial"
 *
 * Todas as contas aqui compartilham a senha `automatize-dev-2026`, escrita em
 * texto plano neste arquivo e documentada no README. Isso é aceitável num
 * banco local descartável e é exatamente o tipo de coisa que não pode ir para
 * produção: qualquer pessoa que leia o repositório teria login de "vendedor
 * verificado" na plataforma de verdade.
 *
 * Produção usa `prisma/seed.ts`, que semeia só o que é seguro e necessário
 * (categorias) e não cria nenhuma conta.
 *
 * ## Idempotente
 *
